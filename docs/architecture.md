...
Terraform owns durable infrastructure, not PZ installation details. Its cloud-init payload installs Docker, verifies and mounts the independently created EBS volume, checks out an exact Git commit, and calls `scripts/linux/install-host.sh`. Application behavior remains reviewable and testable outside HCL.
...
8. `pz-stack.service` grows ext4 if EBS was enlarged, validates Compose, and launches the container without treating temporary RCON unavailability as permission to stop it.
...
Bootstrap network/package/Git operations have explicit deadlines. A cloud-init or game-readiness failure leaves the host online for SSM diagnosis because management failure cannot prove zero players. Operators should configure an AWS Billing alarm and use the guarded stop helper after diagnosis.

## Graceful Container Stop

The PZ process is attached to a private FIFO used as its local admin console. On `SIGTERM`, the entrypoint sends `save`, waits `SHUTDOWN_SAVE_SECONDS`, sends `quit`, and waits up to `SHUTDOWN_GRACE_SECONDS`. Only a hung process is escalated to `SIGTERM`/`SIGKILL`; console-write failure or escalation makes PID 1 exit nonzero. Host automation captures the original container ID and accepts only a stopped, non-OOM, exit-0 state. Compose's 120-second stop grace exceeds the default internal sequence.
...
Verified on 2026-08-17 against current primary/official sources:

- The [official PZwiki dedicated-server guide](https://pzwiki.net/wiki/Dedicated_server) identifies dedicated-server Steam App ID `380870`, anonymous SteamCMD installation, Linux `start-server.sh`, `$HOME/Zomboid` data, stable Build 42, and public `16261/udp` plus `16262/udp`.
...
- The [official PZwiki startup reference](https://pzwiki.net/wiki/Startup_parameters) identifies `-servername`, `-adminusername`, `-adminpassword`, `-statistic`, and JVM memory semantics.
- The [Project Zomboid site](https://projectzomboid.com/blog/news/2026/) reported Build 42 as stable at verification time.
