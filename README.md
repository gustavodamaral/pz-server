...
| --- | --- |
| Terraform | Network, security group, IAM/SSM, EC2, and independent persistent EBS volume |
...
AWS prices and policies change. Check the current AWS Pricing Calculator rather than treating any old estimate as permanent. Optional EC2 detailed monitoring is off by default.
...
7. Gracefully stop the one exact Compose container, require `Running=false`, `OOMKilled=false`, and exit code `0`, then initiate Linux poweroff immediately. Missing, multiple, changed, or uninspectable containers are unknown, never proof of a stop.
...
| Normal target memory | `2.0` | `14g` | `4g` | `12g` |
...
# Set repository_ref to: git rev-parse HEAD
...
2. Wait for readiness, confirm the new server has no players, and stop it gracefully.
3. Set `initialize_blank_data_volume=false`, save a new plan, and apply it. Because user data is immutable first-boot input, this replaces only the disposable EC2/root volume and reattaches the now-formatted protected EBS volume.
4. Keep the value `false` forever after, including snapshot restores and all replacement hosts.
...
2. Run `terraform -chdir=terraform destroy -target=aws_volume_attachment.world` to detach it cleanly.
3. Run `terraform -chdir=terraform state rm aws_ebs_volume.world` to leave it unmanaged and preserved.
4. Run the normal destroy and record the retained volume/snapshot ID.
